...
export const generateTags = async (
    ...
    const res = await fetch(`${WEBUI_BASE_URL}/api/v1/tasks/tags/completions`, {
        ...
        .then(async (res) => {
            // Be defensive: backend might return HTML on errors/timeouts
            const contentType = res.headers.get('content-type') || '';
            if (!res.ok) {
                const body = contentType.includes('application/json')
                    ? await res.json().catch(() => ({ detail: `HTTP ${res.status}` }))
                    : await res.text().then((t) => ({ detail: t }))
                          .catch(() => ({ detail: `HTTP ${res.status}` }));
                throw body;
            }
            if (!contentType.includes('application/json')) {
                const text = await res.text();
                throw { detail: `Expected JSON but received non-JSON response: ${text.slice(0, 200)}` };
            }
            return res.json();
        })
		.catch((err) => {
