<script lang="ts">
	...
	const submitHandler = async () => {
		...
		console.log('提交权限组数据:', group);
		console.log('权限详情:', permissions);
		console.log('workspace权限:', permissions.workspace);
		console.log('tickets_view_all:', permissions.workspace?.tickets_view_all);
	...
	const init = () => {
		if (group) {
			...
			// Only initialize permissions if they don't exist, preserve existing settings
			if (!permissions || Object.keys(permissions).length === 0) {
				permissions = {
					workspace: {
						...
						analytics: false,
						...group?.permissions?.workspace
					},
					sharing: { ...group?.permissions?.sharing },
					chat: { ...group?.permissions?.chat },
					features: { ...group?.permissions?.features }
				};
			} else {
				// Merge with existing permissions, preserving user settings
				permissions = {
					workspace: { 
						tickets: false,
						tickets_execute: false,
					...
					sharing: { ...permissions.sharing, ...group?.permissions?.sharing },
					chat: { ...permissions.chat, ...group?.permissions?.chat },
					features: { ...permissions.features, ...group?.permissions?.features }
				};
			}
	...
	$: if (show) {
		// Only init if we have a group (not for default permissions)
		if (group) {
			init();
		}
	}
