<script lang="ts">
	...
	// Ensure permissions has the correct structure on mount
	onMount(() => {
		if (!permissions.workspace) permissions.workspace = {};
		if (!permissions.sharing) permissions.sharing = {};
		if (!permissions.chat) permissions.chat = {};
		if (!permissions.features) permissions.features = {};
		
		// Ensure workspace permissions have all required fields
		if (permissions.workspace.tickets_view_all === undefined) {
			permissions.workspace.tickets_view_all = false;
		}
		if (permissions.workspace.tickets_execute === undefined) {
			permissions.workspace.tickets_execute = false;
		}
		if (permissions.workspace.tickets === undefined) {
			permissions.workspace.tickets = false;
		}
		if (permissions.workspace.analytics === undefined) {
			permissions.workspace.analytics = false;
		}
	});
</script>
...
<div>
	...
	<div>
		...
		<div class="  flex w-full justify-between my-2 pr-2">
			<div class=" self-center text-xs font-medium">
				{$i18n.t('Tickets Access')}
			</div>
			<Switch bind:state={permissions.workspace.tickets} />
		</div>

		<div class="  flex w-full justify-between my-2 pr-2">
			<div class=" self-center text-xs font-medium">
				{$i18n.t('Tickets Execute')}
			</div>
			<Switch bind:state={permissions.workspace.tickets_execute} />
		</div>

		<div class="  flex w-full justify-between my-2 pr-2">
			<div class=" self-center text-xs font-medium">
				{$i18n.t('Tickets View All')}
			</div>
			<Switch bind:state={permissions.workspace.tickets_view_all} />
		</div>
