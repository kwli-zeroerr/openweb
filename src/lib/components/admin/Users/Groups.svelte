<script>
	...
	const updateDefaultPermissionsHandler = async (group) => {
		console.debug('=== 开始保存默认权限 ===');
		console.debug('传入的 group:', group);
		console.debug('group.permissions:', group.permissions);
		console.debug('group.permissions.features:', group.permissions?.features);
		console.debug('group.permissions.features.tickets:', group.permissions?.features?.tickets);
		console.debug('当前 defaultPermissions:', defaultPermissions);
		console.debug('当前 defaultPermissions.features.tickets:', defaultPermissions.features?.tickets);
		...
		if (res) {
			console.debug('保存成功，API 响应:', res);
			console.debug('API 响应中的 features:', res.features);
			console.debug('API 响应中的 features.tickets:', res.features?.tickets);
			toast.success($i18n.t('Default permissions updated successfully'));
			
			// 更新 defaultPermissions - 使用深拷贝确保响应式更新
			defaultPermissions = JSON.parse(JSON.stringify(res));
			console.debug('更新后的 defaultPermissions:', defaultPermissions);
			console.debug('更新后的 defaultPermissions.features.tickets:', defaultPermissions.features?.tickets);
		} else {
			console.error('保存失败，res 为 null');
		}
		console.debug('=== 保存默认权限结束 ===');
	};
