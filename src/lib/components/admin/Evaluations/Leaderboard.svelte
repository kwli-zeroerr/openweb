<script lang="ts">
	...
	import ModelScoringLeaderboard from '../ModelScoring/ModelScoringLeaderboard.svelte';
	...
	let rankedModels = [];
	let activeTab = 'arena'; // 'arena' or 'scoring'
...
<div class="flex flex-col space-y-4">
	...
	<div class="flex space-x-1 bg-gray-100 dark:bg-gray-800 rounded-lg p-1">
		<button
			class="flex-1 px-4 py-2 text-sm font-medium rounded-md transition-colors {activeTab === 'arena' 
				? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 shadow-sm' 
				: 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100'}"
			on:click={() => activeTab = 'arena'}
		>
			竞技场评分
		</button>
		<button
			class="flex-1 px-4 py-2 text-sm font-medium rounded-md transition-colors {activeTab === 'scoring' 
				? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 shadow-sm' 
				: 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100'}"
...
<div class="scrollbar-hidden relative whitespace-nowrap overflow-x-auto max-w-full rounded-sm">
					...
					>
						...
						<td class="px-3 py-1.5 flex flex-col justify-center">
							<div class="flex items-center gap-2">
								<div class="shrink-0">
									<img
										src={model?.info?.meta?.profile_image_url ?? `${WEBUI_BASE_URL}/zeroerr-logo.jpg`}
										alt={model.name}
	...
	{:else if activeTab === 'scoring'}
		<!-- 数据库评分内容 -->
		<ModelScoringLeaderboard {feedbacks} />
	{/if}
</div>
