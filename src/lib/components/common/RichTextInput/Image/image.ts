...
export const Image = Node.create<ImageOptions>({
	...
	addNodeView() {
		return ({ node, editor }) => {
			...
			// 添加样式，使图片可以被选中和删除
			img.classList.add('rounded-md', 'max-h-72', 'w-fit', 'object-contain', 'cursor-pointer');
			...
			img.addEventListener('click', (e) => {
				...
				if (pos !== null && pos >= 0) {
					...
					// 使用 TextSelection 来正确选中图片节点
					const { TextSelection } = state.schema;
					const selection = TextSelection.near($pos);
					const tr = state.tr.setSelection(selection);
					view.dispatch(tr);
					// 使用 requestAnimationFrame 确保焦点设置正确
