<script lang="ts">
	...
	export let unarchiveHandler: ((chatId: string) => void) | null = null;
	...
	<div>
		...
		<div class="flex flex-col w-full px-5 pb-4 dark:text-gray-200">
			...
			<div class=" flex flex-col w-full sm:flex-row sm:justify-center sm:space-x-6">
				{#if chatList}
					<div class="w-full">
						...
						<div class="text-left text-sm w-full mb-3 max-h-[22rem] overflow-y-scroll">
							...
							{#each chatList as chat, idx (chat.id)}
								...
								>
									...
									>
										...
										<!-- 反馈信息 -->
										{#if chat.thumbs_up > 0 || chat.thumbs_down > 0}
											<div class="flex items-center gap-2 mt-1">
												{#if chat.thumbs_up > 0}
													<div class="flex items-center gap-1 text-green-600 dark:text-green-400">
														<ArrowUpCircle className="w-3 h-3" />
														<span class="text-xs">{chat.thumbs_up}</span>
													</div>
												{/if}
												{#if chat.thumbs_down > 0}
													<div class="flex items-center gap-1 text-red-600 dark:text-red-400">
														<ExclamationTriangle className="w-3 h-3" />
														<span class="text-xs">{chat.thumbs_down}</span>
													</div>
												{/if}
											</div>
										{/if}
							...
							{#if !allChatsLoaded && loadHandler && chatList.length > 0}
								<div class="flex justify-center py-4">
									<button
										class="px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
										on:click={() => loadHandler && loadHandler()}
									>
										Load More
									</button>
								</div>
				...
				{:else}
					<div class="flex justify-center items-center min-h-20 w-full h-full">
						<Loader />
					</div>
				{/if}
			</div>
