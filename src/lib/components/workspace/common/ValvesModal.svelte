<script lang="ts">
	...
	const initHandler = async () => {
		...
		try {
			...
			// Debug: log if valvesSpec is null
			if (!valvesSpec && userValves) {
				console.warn(`No user valves spec found for ${type} ${id}. This means UserValves class may not be defined in the tool/function code.`);
			}
		} catch (e) {
			console.error(`Error fetching valves for ${type} ${id}:`, e);
