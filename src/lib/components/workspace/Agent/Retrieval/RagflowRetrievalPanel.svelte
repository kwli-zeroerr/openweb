...
<script lang="ts">
  ...
  import { agentAPI, type RagFlowRetrievalRequest, type RagFlowRetrievalResponse, type RagFlowDataset } from "$lib/apis/agent";
  ...
  async function loadDatasets() {
    try {
      ...
      const response = await agentAPI.ragflowListDatasets();
  ...
  async function performRetrieval() {
    ...
    try {
      ...
      const result = await agentAPI.ragflowRetrieval(req);
