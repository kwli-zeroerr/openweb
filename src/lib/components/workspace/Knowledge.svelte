...
{#if loaded}
    <DeleteConfirmDialog
        ...
        input={true}
        inputPlaceholder={'输入：确定删除'}
        on:confirm={(e) => {
            const value = e.detail?.trim?.() ?? e.detail;
            if (value !== '确定删除') {
                toast.error($i18n.t("Please type '确定删除' to confirm."));
                return;
            }
	...
	<div class="flex flex-col gap-1 my-1.5">
		...
		<div class=" flex w-full space-x-2">
			...
			<div>
				<button
					class="px-4 py-2 rounded-lg bg-black hover:bg-gray-800 text-white transition font-medium text-sm flex items-center space-x-1"
