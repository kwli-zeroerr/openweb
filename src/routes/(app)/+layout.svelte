<script lang="ts">
	...
	import ZeroErrInfoModal from '$lib/components/ZeroErrInfoModal.svelte';
	...
	onMount(async () => {
		if ($user === undefined || $user === null) {
			...
			]).then(([bannersData, toolsData]) => {
				banners.set(bannersData);
				tools.set(toolsData);
			}).catch((error) => {
				console.error('Failed to load banners or tools:', error);
			});

			// 异步加载工具服务器数据
			getToolServersData($settings?.toolServers ?? []).then((toolServersData) => {
