<script lang="ts">
	...
	const loadFeedbackData = async () => {
		...
		try {
			...
			const response = await fetch(`${WEBUI_API_BASE_URL}/chats/share/${$page.params.id}/feedback`, {
				method: 'GET',
				headers: {
			...
			if (response.ok) {
				...
				console.warn('Failed to load feedback data:', response.status);
			}
		} catch (error) {
			...
			<div class=" flex flex-col w-full flex-auto overflow-auto h-0" id="messages-container">
				...
				>
					<div class="px-3">
						...
						<!-- 反馈数据统计 -->
						{#if feedbackData && feedbackData.feedback_count > 0}
							<div class="mt-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
								<div class="flex items-center space-x-4 text-sm">
									<div class="flex items-center space-x-1">
										<span class="text-green-600">👍</span>
										<span class="font-medium">{feedbackData.thumbs_up}</span>
									</div>
									<div class="flex items-center space-x-1">
										<span class="text-red-600">👎</span>
										<span class="font-medium">{feedbackData.thumbs_down}</span>
									</div>
									<div class="flex items-center space-x-1">
										<span class="text-blue-600">💬</span>
										<span class="font-medium">{feedbackData.feedback_count}</span>
									</div>
						...
						{:else if feedbackLoading}
							<div class="mt-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
								<div class="text-sm text-gray-500">加载反馈数据中...</div>
							</div>
						{/if}
					</div>
				</div>
