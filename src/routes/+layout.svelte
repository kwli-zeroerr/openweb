				...
				if (done) {
					...
					if ($isLastActiveTab) {
						if ($settings?.notificationEnabled ?? false) {
							new Notification(`${title} • ZeroErr GPT`, {
								body: content,
								icon: `${WEBUI_BASE_URL}/zeroerr-logo.jpg`
							});
						}
					}
	...
	const channelEventHandler = async (event) => {
		...
		if ((!channel || isFocused) && event?.user?.id !== $user?.id) {
			...
			if (type === 'message') {
				if ($isLastActiveTab) {
					if ($settings?.notificationEnabled ?? false) {
						new Notification(`${data?.user?.name} (#${event?.channel?.name}) • ZeroErr GPT`, {
							body: data?.content,
							icon: data?.user?.profile_image_url ?? `${WEBUI_BASE_URL}/zeroerr-logo.jpg`
						});
					}
				}
...
<svelte:head>
	...
	<link crossorigin="anonymous" rel="icon" href="{WEBUI_BASE_URL}/zeroerr-logo.jpg" />
