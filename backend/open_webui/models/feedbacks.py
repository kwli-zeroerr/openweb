...
class Feedback(Base):
    ...
    user_id = Column(Text)
    chat_id = Column(Text)  # 添加chat_id字段
...
class FeedbackModel(BaseModel):
    id: str
    user_id: str
    chat_id: Optional[str] = None  # 添加chat_id字段
    version: int
    type: str
...
class FeedbackTable:
    ...
    def get_feedback_by_chat_id(self, chat_id: str) -> list[FeedbackModel]:
        """根据chat_id获取所有反馈数据"""
        try:
            with get_db() as db:
                feedbacks = db.query(Feedback).filter_by(chat_id=chat_id).all()
                return [FeedbackModel.model_validate(feedback) for feedback in feedbacks]
        except Exception as e:
            log.error(f"Error getting feedback by chat_id {chat_id}: {e}")
            return []
