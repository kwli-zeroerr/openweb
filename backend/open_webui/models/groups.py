...
class GroupTable:
    ...
    def get_group_members(self, id: str) -> list[GroupMember]:
        try:
            user_ids = self.get_group_user_ids_by_id(id) or []
            # Normalize to list
            if not isinstance(user_ids, list):
                return []
            return [GroupMember(user_id=uid) for uid in user_ids if isinstance(uid, str)]
        except Exception:
            return []
