...
async def get_tools(request: Request, user=Depends(get_verified_user)):
    # 获取用户可访问的工具（包括默认工具）
    user_tools = Tools.get_tools_by_user_id(user.id, permission="read")
    ...
    # 注意：user_tools 已经通过 get_tools_by_user_id 过滤过了，包含了默认工具
    # 所以这里不需要再次过滤，否则会过滤掉默认工具
    # 但是 Tool Servers 需要根据 access_control 过滤
    if user.role == "admin" and BYPASS_ADMIN_ACCESS_CONTROL:
        # Admin can see all tools
        return tools
    else:
        # 只过滤 Tool Servers，不过滤已经通过 get_tools_by_user_id 获取的工具
        # 因为 get_tools_by_user_id 已经包含了默认工具
        user_group_ids = {group.id for group in Groups.get_groups_by_member_id(user.id)}
        
        # 分离普通工具和 Tool Servers
        regular_tools = [tool for tool in tools if not tool.id.startswith("server:")]
        tool_servers = [tool for tool in tools if tool.id.startswith("server:")]
        ...
        # 返回普通工具（已包含默认工具）+ 过滤后的 Tool Servers
        return regular_tools + filtered_tool_servers
...
async def update_tools_by_id(
    ...
    # Only admins can set is_default_for_all_users
    if form_data.is_default_for_all_users and user.role != "admin":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only administrators can set tools as default for all users",
        )
    ...
    try:
        ...
        # Only admins can modify is_default_for_all_users and default_for_group_ids
        updated_data = form_data.model_dump(exclude={"id"})
        if user.role != "admin":
            # Non-admins cannot change these fields, keep existing values
            existing_tool = Tools.get_tool_by_id(id)
            if existing_tool:
                updated_data["is_default_for_all_users"] = existing_tool.is_default_for_all_users
                updated_data["default_for_group_ids"] = existing_tool.default_for_group_ids
        ...
        log.debug(f"Updating tool {id} with data: {updated}")
        log.debug(f"default_for_group_ids: {updated.get('default_for_group_ids')}")
...
async def get_tools_user_valves_spec_by_id(
    ...
    tools = Tools.get_tool_by_id(id)
    if tools:
        # Get the module from sys.modules since UserValves is at module level
        import sys
        module_name = f"tool_{id}"
        
        # Always reload the module to ensure we have the latest code
        if module_name in sys.modules:
            del sys.modules[module_name]
        
        # Load the module
        tools_instance, _ = load_tool_module_by_id(id)
        request.app.state.TOOLS[id] = tools_instance
        
        # Get the module from sys.modules after loading
        if module_name in sys.modules:
            tools_module = sys.modules[module_name]
            log.debug(f"Module {module_name} loaded, has UserValves: {hasattr(tools_module, 'UserValves')}")
            log.debug(f"Module attributes: {dir(tools_module)}")
        else:
            log.warning(f"Module {module_name} not found in sys.modules after loading")
            return None

        if hasattr(tools_module, "UserValves"):
            UserValves = tools_module.UserValves
            try:
                # Try new Pydantic v2 method first, fallback to v1
                if hasattr(UserValves, "model_json_schema"):
                    return UserValves.model_json_schema()
                else:
                    return UserValves.schema()
            except Exception as e:
                log.exception(f"Error getting UserValves schema: {e}")
                return None
        else:
            log.warning(f"UserValves not found in module {module_name}")
            return None
    else:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
async def update_tools_user_valves_by_id(
    ...
    if tools:
        ...
        if module_name in sys.modules:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=ERROR_MESSAGES.DEFAULT("UserValves not found"),
            )
