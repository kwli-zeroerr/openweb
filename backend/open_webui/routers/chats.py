...
@router.get("/list/user/{user_id}/with-feedback", response_model=list[dict])
async def get_user_chat_list_with_feedback(
    user_id: str,
    page: Optional[int] = None,
    query: Optional[str] = None,
    order_by: Optional[str] = None,
    ...
    """获取用户聊天列表及其反馈统计信息"""
    # 检查权限：管理员或有workspace.analytics权限的用户
    if not (user.role == 'admin' or (user.permissions and user.permissions.get('workspace', {}).get('analytics', False))):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=ERROR_MESSAGES.ACCESS_PROHIBITED,
        )
    ...
    return Chats.get_chat_list_by_user_id_with_feedback(
        user_id, include_archived=True, filter=filter, skip=skip, limit=limit
    )
...
############################
# GetSharedChatFeedback
############################


@router.get("/share/{share_id}/feedback")
async def get_shared_chat_feedback(share_id: str, user=Depends(get_verified_user)):
    """获取分享聊天的反馈数据"""
    if user.role == "pending":
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail=ERROR_MESSAGES.NOT_FOUND
    ...
    try:
        ...
        feedback_list = Feedbacks.get_feedback_by_chat_id(chat.id)
        ...
        for feedback in feedback_list:
            rating = feedback.data.get('rating') if feedback.data else None
            if rating == 1:
                thumbs_up += 1
            elif rating == -1:
                thumbs_down += 1
            ...
            feedback_detail = {
                ...
                "rating": rating,
                "reason": feedback.data.get('reason') if feedback.data else None,
                "comment": feedback.data.get('comment') if feedback.data else None,
                "tags": feedback.data.get('tags') if feedback.data else [],
                "message_id": feedback.meta.get('message_id') if feedback.meta else None,
        ...
        log.error(f"Error getting shared chat feedback: {e}")
        raise HTTPException(
...
async def update_chat_message_by_id(
    ...
    try:
        ...
        AnalyticsService.log_chat_message(
            user_id=user.id,
            chat_id=id,
        ...
        log.error(f"Failed to log analytics event: {e}")
