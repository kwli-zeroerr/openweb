...
WEBUI_NAME = os.environ.get("WEBUI_NAME", "ZeroErr GPT")
if WEBUI_NAME != "ZeroErr GPT":
    WEBUI_NAME += " (ZeroErr GPT)"

WEBUI_FAVICON_URL = "https://openwebui.com/zeroerr-logo.jpg"
...
def parse_section(section):
    ...
    for li in section.find_all("li"):
        ...
        for link in li_copy.find_all("a"):
            link.unwrap()  # Remove the <a> tag but keep the text
        
        # Extract raw HTML string without links
        raw_html = str(li_copy)

        # Extract text without HTML tags
        text = li.get_text(separator=" ", strip=True)
        
        # Clean up any remaining link text patterns
        import re
        text = re.sub(r'\b(Commit|Docs|#\d+)\b', '', text)
...
WEBUI_BUILD_HASH = os.environ.get("WEBUI_BUILD_HASH", "1.2.0")
...
####################################
# Local config.json (no system env leaks)
####################################

# Allow users to put secrets into DATA_DIR/config.json
# Example: { "DINGTALK_WEBHOOK_SECRET": "SEC..." }
