...
frontend_favicon = FRONTEND_BUILD_DIR / "static" / "zeroerr-logo.jpg"

if frontend_favicon.exists():
    try:
        shutil.copyfile(frontend_favicon, STATIC_DIR / "zeroerr-logo.jpg")
    except Exception as e:
        logging.error(f"An error occurred: {e}")
        ...
        if r.ok:
            if "logo" in data:
                ...
                if r.status_code == 200:
                    with open(f"{STATIC_DIR}/zeroerr-logo.jpg", "wb") as f:
...
if default_prompt_suggestions == []:
    default_prompt_suggestions = [
        {
            "title": ["eCoder编码器", "LED状态都表示什么意思？"],
            "content": "eCoder的LED状态都表示什么意思？",
        },
        {
            "title": ["eRob机器人关节模组", "有没有力控相关的教程？"],
            "content": "有没有力控相关的教程啊？",
        },
        {
            "title": ["Github相关问题", "有没有开源主站的使用教程？"],
            "content": "有没有开源主站的使用教程啊？",
        },
        {
            "title": ["资料下载问题", "去哪可以下载关节的资料？"],
            "content": "去哪可以下载关节的资料？",
        },
        {
            "title": ["报错0x8400", "如何处理？"],
            "content": "报错0x8400如何处理？",
        },
        {
            "title": ["0x03对应的", "主站请求消息帧格式是什么？"],
            "content": "0x03对应的主站请求消息帧格式是什么？",
        },
    ]
...
USER_PERMISSIONS_FEATURES_TICKETS = (
    os.environ.get("USER_PERMISSIONS_FEATURES_TICKETS", "False").lower() == "true"
)

USER_PERMISSIONS_WORKSPACE_TICKETS_ACCESS = (
    os.environ.get("USER_PERMISSIONS_WORKSPACE_TICKETS_ACCESS", "False").lower() == "true"
)

USER_PERMISSIONS_WORKSPACE_TICKETS_EXECUTE = (
    os.environ.get("USER_PERMISSIONS_WORKSPACE_TICKETS_EXECUTE", "False").lower() == "true"
)

USER_PERMISSIONS_WORKSPACE_TICKETS_VIEW_ALL = (
    os.environ.get("USER_PERMISSIONS_WORKSPACE_TICKETS_VIEW_ALL", "False").lower() == "true"
)
...
DEFAULT_USER_PERMISSIONS = {
    "workspace": {
        ...
        "tickets": USER_PERMISSIONS_WORKSPACE_TICKETS_ACCESS,
        "tickets_execute": USER_PERMISSIONS_WORKSPACE_TICKETS_EXECUTE,
        "tickets_view_all": USER_PERMISSIONS_WORKSPACE_TICKETS_VIEW_ALL,
    },
    "sharing": {
    ...
    "features": {
        ...
        "tickets": USER_PERMISSIONS_FEATURES_TICKETS,
    },
}
...
DEFAULT_ARENA_MODEL = {
    ...
    "name": "Arena Model",
    "meta": {
        "profile_image_url": "/zeroerr-logo.jpg",
...
SEARCH_KNOWLEDGE_TOOL_PROMPT_TEMPLATE = PersistentConfig(
    "SEARCH_KNOWLEDGE_TOOL_PROMPT_TEMPLATE",
    "task.search_knowledge.prompt_template",
    os.environ.get("SEARCH_KNOWLEDGE_TOOL_PROMPT_TEMPLATE", ""),
)


DEFAULT_SEARCH_KNOWLEDGE_TOOL_PROMPT_TEMPLATE = """【角色设定 Role】
...
4.下一步行动: 说明后续处理流程、时间预期，或需要客户提供的额外信息。
