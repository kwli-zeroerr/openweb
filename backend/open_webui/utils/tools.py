...
async def get_tools(
    ...
    for tool_id in tool_ids:
        ...
        if tool is None:
            ...
            tools_instance = request.app.state.TOOLS.get(tool_id, None)
            if tools_instance is None:
                tools_instance, _ = load_tool_module_by_id(tool_id)
                request.app.state.TOOLS[tool_id] = tools_instance

            extra_params["__id__"] = tool_id

            # Get the module from sys.modules to access UserValves (which is at module level)
            import sys
            module_name = f"tool_{tool_id}"
            module = sys.modules.get(module_name, None)
            
            # Set valves for the tool
            if module and hasattr(module, "valves") and hasattr(module, "Valves"):
                valves = Tools.get_tool_valves_by_id(tool_id) or {}
                module.valves = module.Valves(**valves)
            if module and hasattr(module, "UserValves"):
                user_valves_dict = Tools.get_user_valves_by_id_and_user_id(tool_id, user.id)
                if user_valves_dict is None:
                    user_valves_dict = {}
            ...
            for spec in tool.specs:
                ...
                function_name = spec["name"]
                # Get function from Tools instance, not from module
                tool_function = getattr(tools_instance, function_name)
                callable = get_async_tool_function_and_apply_extra_params(
